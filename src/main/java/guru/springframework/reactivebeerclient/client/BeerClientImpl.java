...
public class BeerClientImpl implements BeerClient {
    ...
    public Mono<BeerPagedList> listBeers(Integer pageNumber, Integer pageSize, String beerName, String beerStyle, Boolean showInventoryOnhand) {
        return this.webClient.get().uri(uriBuilder -> {
            return uriBuilder.path(WebClientConfigProperties.BEER_V2_URL)
    ...
    public Mono<ResponseEntity<Void>> createBeer(BeerDto beerDto) {
        return webClient.post().uri(WebClientConfigProperties.BEER_V2_URL).body(BodyInserters.fromValue(beerDto)).retrieve().toBodilessEntity();
    }

    @Override
    public Mono<ResponseEntity<Void>> updateBeer(UUID uuid, BeerDto beerDto) {
        return webClient.put().uri(uriBuilder -> uriBuilder.path(WebClientConfigProperties.BEER_V2_URL_BY_BEERID).build(uuid))
                .body(BodyInserters.fromValue(beerDto)).retrieve().toBodilessEntity();
    }

    @Override
    public Mono<ResponseEntity<Void>> deleteBeerById(UUID id) {
        return webClient.delete().uri(uriBuilder -> uriBuilder.path(WebClientConfigProperties.BEER_V2_URL_BY_BEERID).build(id))
                .retrieve().toBodilessEntity();
    }

    @Override
    public Mono<BeerDto> getBeerByUPC(String upc) {
        return webClient.get().uri(uriBuilder -> uriBuilder.path(WebClientConfigProperties.BEER_V2_URL_BY_UPC).build(upc)).retrieve().bodyToMono(BeerDto.class);
    }
}
